...
library(ggplot2)
library(ggpubr)
setwd("/home/weider/Genome_assemblies/Marey_maps")
...
chrom1 <- ggplot(CHR1, aes(x=Physical_position)) + 
  ...
  theme_light() + theme(axis.title.x=element_blank(),
                        axis.text.x=element_blank(), legend.title = element_blank(), axis.title.y=element_blank())
chrom1
...
chrom2 <- ggplot(CHR2, aes(x=Physical_position)) + 
  ...
  xlab("Phyiscal Position (MB)")+ ylab("Genetic Position (cM)")+ theme_light()+theme(axis.title.x=element_blank(),
                                                                                     axis.text.x=element_blank(), legend.title = element_blank(), axis.title.y=element_blank())
chrom2
#run window recomb rate estimates
...
marey_stack <- ggarrange(chrom1, chrom2, chrom3, chrom4, chrom5, chrom6, chrom7, chrom8, chrom9, chrom10, chrom11, chrom12, ncol = 3, nrow = 4, legend = "top", common.legend = T)
marey_stack 

annotate_figure(marey_stack, top = text_grob("Marey Maps", color = "black", face = "bold", size = 20), bottom = text_grob("Physical Position (MB)", color = "black", size = 15), 
                left = text_grob("Genetic Position (cM)", color = "black", size =15, rot = 90))
